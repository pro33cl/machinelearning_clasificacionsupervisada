...
class ClasificadorSupervisado:
    ...
    def crear_folds(self, metodo='holdout', test_size=0.3, k=5, random_state=42):
        self.folds = {}
        if metodo == 'holdout':
    ...
    def calcular_metricas_promedio(metricas_por_modelo):
        metricas_promedio = {}
    ...
    def entrenar_modelos(self, hiperparametros=None):

        if hiperparametros is None:
            hiperparametros = {}

        # Inicializar modelos con hiperparámetros personalizados o por defecto
        algoritmos = {
        ...
        for nombre_alg, modelo in algoritmos.items():
            ...
            for fold, datos in self.folds.items():
                ...
                y_proba = modelo.predict_proba(datos['X_val'])[:, 1] if hasattr(modelo, 'predict_proba') else y_pred
    ...
    def predecir_por_fold(self, X):
        ...
        for nombre_alg, modelos_por_fold in self.modelos_entrenados.items():
            predicciones[nombre_alg] = {}

            for fold, modelo in modelos_por_fold.items():
    ...
    def graficar_metricas_seaborn(self):
        ...
        for alg, resultados in self.metricas_por_modelo.items():
            datos = []
